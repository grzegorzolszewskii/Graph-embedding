...
def train(graph, model, optimizer, epochs=200, max_loss=3.5, m_rows=10, m_cols=52):
    ...
    for epoch in range(epochs):
        for batch in range(0, int(len(graph) // m_rows)):
            inputs = th.randint(nodes_num, (m_rows, m_cols))    # th.randint bierze lizcbe z przedzialu [0, a)
            for i in range(0, m_rows):                      # random.randint bierze liczbe z przedzialu [a, b]
                node = m_rows * batch + i
                node_connected = rand.choice(tuple(graph[node]))
                inputs[i, 0] = node
                inputs[i, 1] = node_connected
                for j in range(2, m_cols):      # od 2 kolumny tylko wierzcholki niepolaczone
            ...
            optimizer.zero_grad()       # za kazdym nowy gradient
            preds = model(inputs)
            ...
            loss.backward()     # obliczam pochodnych
            optimizer.step()    # odejmuje pochodne do pozycji wierzcholkow
